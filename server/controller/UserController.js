...
const saveUser = async (req, res) => {
  ...
  try {
    ...
    const hashedcPassowrd = await bycrypt.hash(cPassowrd, salt);
...
const loginUser = async (req, res) => {
  ...
  if (findUser && (await bycrypt.compare(password, findUser.password))) {
    res
      .status(200)
      .send({ token: genarateToken(findUser._id), user: findUser });
  } else {
...
const UpdateUserDetails = async (req, res) => {
  ...
  const { name, address, nic, mobile } = req.body;
  ...
  try {
    ...
    await UserModel.findByIdAndUpdate(UpdateID, UpdateDetails)
      .then(() => {
